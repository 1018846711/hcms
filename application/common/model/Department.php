...
class Department extends CommonModel {
    ...
    public function getDepartmentStr($depList) {
        ...
        foreach ($depList as $dep) {
            $checkboxHtml = "<input class='checkbox-dep' type='checkbox' data-id='{$dep["id"]}'>";
            $formHtml = "<i class='layui-icon icon-delete' style='size: 18px'>&#xe640;</i>
						<i class='layui-icon icon-edit' style='size: 18px'>&#xe642;</i>
						<i class='layui-icon icon-add' style='size: 18px'>&#xe654;</i>";
            $module = strtolower(request()->module());
            $controller = strtolower(request()->controller());
            $action = strtolower(request()->action());
            if ("{$module}_{$controller}_{$action}" == "admin_department_index") {
                $checkboxHtml = "";
            }
            if ("{$module}_{$controller}_{$action}" == "admin_administrator_index") {
                $formHtml = "";
            }
